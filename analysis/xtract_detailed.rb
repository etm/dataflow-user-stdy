...
def get_buttons(what,io,sol) #{{{
  ...
  dat = dat.split(',')
  sol = sol.split(',')
  [duration,dat.join(','),sol.join(','),(dat==sol).to_s,(dat&sol).length().to_s(),sol.length().to_s(),(dat-sol).length().to_s()]
end #}}}
...
results = CSV.open('experience_detailed.csv','wb')
...
results = CSV.open('results_detailed.csv','wb')
Dir.glob('finished/*.xes.yaml') do |f|
  io = File.open f
  yaml = YAML.load_stream io
  first = yaml.shift
  ...
  if first.dig('log','trace','cpee:name') == 'QuestSub'
    ...
    if item1.length == 34 && item2.length == 34
